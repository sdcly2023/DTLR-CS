...
def testNet(images_dir):
    ...
    solver.loadtar(
        Constants.weight_dir + 'ResUNet34TargetA')  # cenet\AL-Net_no_alloss_clusteredCell1 /home/chenxiaokai/daima/CE-Net-master/weights/U_Netttrain
    ...
    for ii in range(0, len(testlist)):
        ...
        for image_name in tqdm(images_name):  # 对于每一个文件进行处理image 是文件的名字
            ...
            surface_distances = metrics.compute_surface_distances(true_mask, pred_mask, spacing_mm=(1.0, 1.0))
            hd += metrics.compute_robust_hausdorff(surface_distances, 95)
            print('[test]  AJI: %.4f, instaji: %.4f,PQ: %.4f,instpq:%.4f,HD: %.4f,insthd: %.4f, DC: %.4f,  instdc:%.4f ' % (
            aji, instaji, pq, instpq, hd, insthd, DC, instDC))
            index = index + 1
    aji = aji / index
    pq = pq / index
    hd = hd / index

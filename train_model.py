...
from Load_Dataset import RandomGenerator, ValGenerator, ImageToImage2D,ImageToImage2D_Test,ValGenerator_new
...
def main_loop(batch_size=config.batch_size, model_type='', tensorboard=True):
    ...
    val_tf = ValGenerator_new()
    # val_tf = ValGenerator(output_size=[config.real_size, config.real_size])
    train_dataset = ImageToImage2D(config.train_dataset, train_tf, image_size=config.img_size)
    val_dataset = ImageToImage2D_Test(config.val_dataset, val_tf, image_size=config.img_size)
    ...
    if model_type == 'UCTransNet':
        ...
        logger.info('transformer expand ratio: {}'.format(config_vit.expand_ratio))
        model = UCTransNet(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
        pretrained_UNet_model_path = "BNS_enhance/UNet/Test_session_01.10_16h49_384_384/models/best_model-UNet.pth.tar"
        ...
        state_dict = {k[7:]: v for k, v in pretrained_UNet.items() if k[7:] in model2_dict.keys()}
